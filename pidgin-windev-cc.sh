...
devroot="$1"
path="$2"
env_dependencies="ca-certificates gawk grep make patch tar unzip wget zip git mingw-w64-i686-toolchain"
...
if [[ "$1" = -* || -z "$devroot" || ( -n "$path" && "$path" != --path ) ]]; then echo "
    ...
    Updated 2021 Jeremiah Blanchard
    Licensed under BSD
...
pacman -Syu --noconfirm $env_dependencies
...
# Preparing Pidgin
step "Cloning Pidgin source code"
if git clone -b v2.14.1-release git://github.com/cacticouncil/pidgin "$source_directory" ; then
